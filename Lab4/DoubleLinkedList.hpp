...
void DoubleLinkedList<T>::insert(T value)
{
	if(insertRec(m_front, value) == 0){
		...
		if(isEmpty()){
			...
			freshNode->setNext(m_back);
			m_back->setPrevious(freshNode);
			m_front = freshNode;
		}else{
			freshNode->setNext(m_front);
			m_front->setPrevious(freshNode);
			m_front = freshNode;
		}
		m_size++;
	}else{
	...
	//CODE FOR INSERTING AT THE END OF LIST
	/*if(insertRec(m_front, value) == 0){			
		Node<T>* freshNode = new Node<T>();
		freshNode->setValue(value);
		if(isEmpty()){
...
bool DoubleLinkedList<T>::find(T value){
	return findRec(m_front, value);
}

template <typename T>
bool DoubleLinkedList<T>::findRec(Node<T>* temp, T value){
	if(temp == nullptr){
		return false; //list is empty or we've reached end of list
	}else if(temp->getValue() == value){
		return true; //found the value!
	}else{
		insertRec(temp->getNext(), value); //keep going!
	}
}
